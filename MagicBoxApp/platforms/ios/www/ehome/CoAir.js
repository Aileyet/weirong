define(['text!ehome/CoAir.html', '../base/util', '../base/socketutil','../knobKnob/knobKnob.zepto',"../base/templates/template",'../base/tooltip/tooltip','../base/schema','../base/i18nMain'],
		function(viewTemplate,Util,SocketUtil,knobKnob,_TU,Tooltip,Schema,I18n) {
		    ...
		    var check=null;
			var CoAir=null;
			var viewInitFun, setBtnColorFun, getAirStatuByCurrAndTypeFun;
			var btnName='';
			var img='';
			var lastRatio=0;
			var study;
		return Piece.View.extend({
			...
			action:function(obj){
				...
				study=Piece.Cache.get("study");
				 if(study==1){
				     //SocketUtil.StudyKey(_TU._T.ehome_Air.device, id);
					  Piece.Cache.put("study-key",id);  
					  Piece.Cache.put("study-module", _TU._T.ehome_Air.device);
			...
			checkValue:function()
			{
					  ...
					  if(serial==null||serial=="")
				     {
					  ...
					  var currentDevice = Util.getCurrentDevice(gatewayId, _TU._T.ehome_Air.device,serial);
			...
			setAir:function(id){
				switch(id)
				{
					case "power":
					{
					    if (CoAir.power == "off") {
					        CoAir.power = "on";
					        SocketUtil.SeCoAir("power", "on");
					        setBtnColorFun(true, $("#" + id));
						}
						else
					    {
					        CoAir.power = "off";
					        SocketUtil.SeCoAir("power", "off");
					        setBtnColorFun(false, $("#" + id));
						}
						break;
					}
					case "mode":
					{
						...
						else
						{
						 ...
						 SocketUtil.SeCoAir("mode", CoAir.mode);
						break;
					}
					case "airVolume"://高速即风量
					{
						...
						SocketUtil.SeCoAir("airVolume", CoAir.airVolume);
						break;
					}
					case "airDir"://风向
					{
						...
						SocketUtil.SeCoAir("airDir", CoAir.airDir);
						break;
					}
					case "autoDir"://扫风即是否风向自动
					{
						...
						SocketUtil.SeCoAir("autoDir", CoAir.autoDir);
						break;
					}
					default:"";
				}
			...
			viewInit:function()
			{
						...
						//模式
						var statuObj = getAirStatuByCurrAndTypeFun("mode", CoAir.mode);
						if(statuObj!=null){
						    $("#mode").next(".btnName").text(statuObj.name);
						}
						//风量
						var statuObj = getAirStatuByCurrAndTypeFun("airVolume", CoAir.airVolume);
						if(statuObj!=null){
						    $("#airVolume").next(".btnName").text(statuObj.name);
						}
						//风向
						var statuObj = getAirStatuByCurrAndTypeFun("airDir", CoAir.airDir);
						if(statuObj!=null){
						    $("#airDir").next(".btnName").text(statuObj.name);
						}
						//风向是否自动

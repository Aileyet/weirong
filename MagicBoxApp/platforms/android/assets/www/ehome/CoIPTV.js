define(['text!ehome/CoIPTV.html','../base/util', '../base/socketutil',"../base/templates/template",'../base/tooltip/tooltip','../base/i18nMain'],
	function(viewTemplate,Util,SocketUtil,_TU,Tooltip,I18n) {
		...
		var device = _TU._T.ehome_CoIPTV.device;
		...
		return Piece.View.extend({
			id: 'ehome_CoIPTV',
			events:{
			    'touchstart #power,#tv-mute,#tv-power,#ok,#left,#right,#up,#down,#chUp,#Return,#chDown,#volUp,#volDown,#tvMute,#tvPower,.custom-btn-div': 'action',
			...
			onLoadTemplate:function(){
				_TU._U.setHeader(_TU._T.ehome_CoIPTV);//加载头部导航
				var obj=_TU._T.ehome_CoIPTV.data;
				var TemplateHtml = $(this.el).find("#ehome_CoIPTV_Template").html();
			...
			onShow: function() {
				...
				if (deviceStatus != null && deviceStatus.length > 0) {
				    ...
				    if (CoObj == null) {
				        cusBtns = new Array();
				        CoObj = { "gatewayId": gatewayId, "device": device, "serial": serial, "cusBtns": cusBtns, };
			...
			setSTB: function (id) {
			    ...
			    SocketUtil.SeCoSTBORIPTV(device, id);
			},
		    //根据获取到的设备值设置设备属性
			viewInit: function () {
			    $('.control-btn').html('')

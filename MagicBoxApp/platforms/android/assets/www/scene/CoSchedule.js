define(['text!scene/CoSchedule.html', '../base/util', '../base/socketutil', '../base/schema', "../base/templates/template", "../base/i18nMain", ],
	function (viewTemplate, Util, SocketUtil, Schema, _TU, I18n) {
	    ...
	    var btnback_color = _TU._T.Color.sliderBgColor;
	    return Piece.View.extend({
	        ...
	        onShow: function () {
	            ...
	            if (operationType == "add") {
	                ...
	                if (Util.IsNullOrEmpty(modeIndex) || currentMode == null) {//新增
	                    ...
	                    //添加该模式到本地临时存储
	                    currentMode = {
	                        "loginId": loginId, "img": null, "modeIndex": modeIndex, "calType": calType, "modeName": "", "isStart": 0, "isFixed": 0,
	                ...
	                if (currentMode == null) {
	                    //添加该模式到本地
	                    currentMode = {
	                        "loginId": loginId, "img": null, "modeIndex": modeIndex, "calType": calType, "modeName": $("#modeName").html(),
	            ...
	            for (var i = 0; i < Schema.PeriodArray.length; i++) {
	                var choose = false;
	                $("<div>")
					.html('<div class="div-list" data-key="' + Schema.PeriodArray[i].key + '"data-name="' + Schema.PeriodArray[i].name + '"><div class="period-list-name">' + Schema.PeriodArray[i].name + '</div><div class="choosedA" style="display:' + (choose ? "block" : "none") + '"><i class="icon iconfont" style="font-size: 20px;color:' + _TU._T.dialog_Style.choosed_color + '">' + _TU._T.dialog_Style.choosed_icon + '</i></div></div>')
	            ...
	            for (var i = 0; i < Schema.WeekArray.length; i++) {
	                var choose = false;
	                $("<div>")
					.html('<div class="div-list" data-key="' + Schema.WeekArray[i].key + '"data-name="' + Schema.WeekArray[i].name + '"><div class="week-list-name">' + Schema.WeekArray[i].name + '</div><div class="choosedA" style="display:' + (choose ? "block" : "none") + '"><i class="icon iconfont" style="font-size: 20px;color' + _TU._T.dialog_Style.choosed_color + '">' + _TU._T.dialog_Style.choosed_icon + '</i></div></div>')
	        ...
	        afterNew: function (obj) {
	            ...
	            switch (Item.device.toLocaleLowerCase()) {
	                ...
	                case "slb0"://电灯
	                    ...
	                    break;
	                case "prj0"://投影仪
	                    CoObj = { modeIndex: modeIndex, calType: calType, gatewayId: Item.gatewayId, serial: Item.serial, device: Item.device, name: Item.name, power: 0, pc: 0, video: 0 };
	                    break;
	            }
	            if (CoObj != null) {
	                btnhtml = this.getBtnHtml(CoObj);
	        ...
	        fillDevice: function (item, btnhtml) {
	            ...
	            currDiv.find(".div-slider").bind("tap", function () {
	                ...
	                if ($(this).find("div").css("float") == "left") {
	                    $(this).css("backgroundColor", btnback_color);
	            ...
	            //投影仪电脑
	            currDiv.find("[data-type=pc]").bind("tap", function () {
	                var value = $(this).attr("value");
	                currentObj = searchCurrentDeviceFun($(this));
	                if (value == 1) {
	                    $(this).attr("value", 0);
	                    ...
	                    setBtnColorFun($(this).next("[data-type=video]"));
	            ...
	            //投影仪视频
	            currDiv.find("[data-type=video]").bind("tap", function () {
	                var value = $(this).attr("value");
	                currentObj = searchCurrentDeviceFun($(this));
	                if (value == 1) {
	                    $(this).attr("value", 0);
	                    ...
	                    setBtnColorFun($(this).prev("[data-type=pc]"));
	                    currentObj.pc = value;
	                }
	                //保存
	        ...
	        loadViewStatus: function (item) {
	            ...
	            if (item.power == 1) {
	                obj.css("backgroundColor", _TU._T.Color.sliderBgColor);
	            ...
	            if (item.autoDir == 1) {
	                obj.attr("value", 1);
	                obj.css("backgroundColor", btnback_color);
	            ...
	            //投影仪电脑
	            obj = currentDiv.find("[data-type=pc]");
	            if (item.pc == 1) {
	                obj.attr("value", 1);
	                obj.css("backgroundColor", btnback_color);
	            }
	            ...
	            //投影仪视频
	            obj = currentDiv.find("[data-type=video]");
	            if (item.video == 1) {
	                obj.attr("value", 1);
	                obj.css("backgroundColor", btnback_color);
	            }

	...
	function(viewTemplate,$, OpenAPI, Util,weatherJson,_TU,SocketUtil) {
		...
		return Piece.View.extend({
			...
			getAirQuality:function(){   //获取空气质量
				var quantizedValue = Piece.Cache.get("quantizedValue");
				if(quantizedValue){
					$("#quantizedValue").text(quantizedValue);
				}else{
					$("#quantizedValue").text("优");
				}
				SocketUtil.AddNewModule("AQ00",function(){
					...
					model.callbackFun=function(sendObj,obj){
						if(obj.command.indexOf("Rspn0000")<0){
							...
							$("#quantizedValue").text(value);
							Piece.Cache.put("quantizedValue",value);
	  					}
					}
			...
			getTemperature:function(){   //获取温度
				var temperature = Piece.Cache.get("temperature");
				if(temperature){
					...
					model.callbackFun=function(sendObj,obj){
						...
						if(obj.command.indexOf("Rspn0000")<0){
							...
							return;
	  						...
	  						console.log(obj);
							var temperature = (parseFloat(obj.temperature)/1000).toFixed(0);
							$("#temperatureValue").text(temperature);
							Piece.Cache.put("temperature",temperature);
	  					}
					}
			...
			getHumidity:function(){  //获取湿度
				var humidity = Piece.Cache.get("humidity");
				if(humidity){
				...
				SocketUtil.AddNewModule("HUM0",function(){
					...
					model.callbackFun=function(sendObj,obj){
						if(obj.command.indexOf("Rspn0000")<0){
							...
							var humidity = (parseFloat(obj.humiture)/1000).toFixed(0);
							$("#humidityValue").text(humidity);
							Piece.Cache.put("humidity",humidity);
	  					}
					}
	                model.queryString={"humiture":""};
	                SocketUtil.sendMessage(model);
				});
			},

...
var app = {
	...
	addDeviceToUI : function(newDevice) {
		...
		//更新设备连接状态
		for (var i = 0; i < $("#div_connected").find(".table-rg").length; i++) {
		if($("#div_connected").find(".table-rg").eq(i).find(".btIp").text()==All.bt_cnn_ip && All.bt_cnn_status==0){
				$("#div_connected").find(".table-rg").eq(i).find(".connFlg").html("YES");
			}
		}
	},
	...
	connectSuccess : function(message) {
		...
		All.bt_cnn_ip=app.device.deviceAddress;
		new Piece.Toast("连接成功！");
	},

	//更新已配对·未配对设备列表
	refreshDeviceUI : function() {
		for (var i = 0; i < $("#div_unconnected").find(".table-rg").length; i++) {
			if($("#div_unconnected").find(".table-rg").eq(i).find(".btIp").text()==app.device.deviceAddress){
				//添加到已配对列表
				$("#div_connected").append($("#div_unconnected").find(".table-rg").eq(i));
				//未配对列表删除该设备
				$("div_unconnected").find(".table-rg").eq(i).remove();
				break;
			}
		}
		//更新设备连接状态
		for (var i = 0; i < $("#div_connected").find(".table-rg").length; i++) {
			if($("#div_connected").find(".table-rg").eq(i).find(".btIp").text()==app.device.deviceAddress){
				$("#div_connected").find(".table-rg").eq(i).find(".connFlg").html("YES");
			}
		}
	},

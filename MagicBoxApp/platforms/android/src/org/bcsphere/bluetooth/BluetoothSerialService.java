...
public class BluetoothSerialService {
    ...
    public synchronized void connect(BluetoothDevice device,String uuidstr, boolean secure) {
        ...
        onBluetoothConnect();//连接手机音频和媒体音频
    }
    ...
    */  
    public void onBluetoothConnect(){  
    	mAdapter.getProfileProxy(MyApplication.getAppContext(), mA2dpProfileListener, BluetoothProfile.A2DP);  
    	mAdapter.getProfileProxy(MyApplication.getAppContext(), mHeadsetProfileListener, BluetoothProfile.HEADSET);  
    }  
    ...
    */  
    public void onBluetoothDisConnect(BluetoothDevice device){  
     if(mBluetoothA2dp!=null){  
         try {  
             disconnect(mBluetoothA2dp.getClass(),mBluetoothA2dp, device);  
     ...
     if(mBluetoothHeadset!=null){  
        try {  
             disconnect(mBluetoothHeadset.getClass(),mBluetoothHeadset, device);  
	...
	private BluetoothProfile.ServiceListener mA2dpProfileListener = new BluetoothProfile.ServiceListener() {  
        public void onServiceConnected(int profile, BluetoothProfile proxy) {  
        	if (profile == BluetoothProfile.A2DP) {  
                ...
                if(mState == STATE_CONNECTING){  
                 try {  
                     connect(mBluetoothA2dp.getClass(),mBluetoothA2dp, mDevice);  
                } catch (Exception e) {  
                	 mAdapter.closeProfileProxy(BluetoothProfile.A2DP, mBluetoothA2dp);  
                     e.printStackTrace();  
                 }  
    ...
    private BluetoothProfile.ServiceListener mHeadsetProfileListener = new BluetoothProfile.ServiceListener() {  
        public void onServiceConnected(int profile, BluetoothProfile proxy) {  
            if (profile == BluetoothProfile.HEADSET) {  
             ...
             if(mState == STATE_CONNECTING){  
                 try {  
                     connect(mBluetoothHeadset.getClass(),mBluetoothHeadset, mDevice);  
                 } catch (Exception e) {  
                	 mAdapter.closeProfileProxy(BluetoothProfile.HEADSET, mBluetoothHeadset);  
                     e.printStackTrace();  
                 }  

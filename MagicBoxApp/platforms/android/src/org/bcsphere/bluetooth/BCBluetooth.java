...
public class BCBluetooth extends CordovaPlugin {
	...
	// when the accept services construct a connection , the service will remove
	// from this map & append into classicalServices map for read/write
	// interface call
	...
	public void initialize(CordovaInterface cordova, CordovaWebView webView) {
		...
		filter3 = new IntentFilter(BluetoothDevice.ACTION_ACL_CONNECTED);
		filter4 = new IntentFilter(
				BluetoothDevice.ACTION_ACL_DISCONNECT_REQUESTED);
		filter5 = new IntentFilter(bluetoothAdapter.ACTION_STATE_CHANGED);
		filter6 = new IntentFilter(
				bluetoothAdapter.ACTION_CONNECTION_STATE_CHANGED);
		// BroadcastReceiver mReceiver;
		myContext.registerReceiver(receiver, filter1);
		myContext.registerReceiver(receiver, filter2);
		myContext.registerReceiver(receiver, filter3);
		myContext.registerReceiver(receiver, filter4);
		myContext.registerReceiver(receiver, filter5);
		myContext.registerReceiver(receiver, filter6);
		...
		try {
				...
				cordova.getThreadPool().execute(new Runnable() {
					...
					public void run() {
						if (action.equals("startScan")) {
							...
							bluetoothAPI
									.getDeviceAllData(json, callbackContext);
			...
			if (action.equals("getBluetoothState")) {
				...
				if (bluetoothAdapter.isEnabled()) {
					...
					Tools.addProperty(obj, Tools.BLUETOOTH_STATE,
							Tools.IS_FALSE);
					callbackContext.success(obj);
				}
				return true;
			...
			if (action.equals("rfcommConnect")) {
				String deviceAddress = Tools
						.getData(json, Tools.DEVICE_ADDRESS);
				...
				BluetoothSerialService classicalService = classicalServices
						.get(deviceAddress);
			...
			if (action.equals("rfcommDisconnect")) {
				String deviceAddress = Tools
						.getData(json, Tools.DEVICE_ADDRESS);
				BluetoothSerialService service = classicalServices
						.get(deviceAddress);
				if (service != null) {
					...
					callbackContext.error("Could not disconnect to "
							+ deviceAddress);
				}
			}
			...
			if (action.equals("rfcommUnListen")) {
				String name = Tools.getData(json, Tools.NAME);
				String uuidstr = Tools.getData(json, Tools.UUID);
				BluetoothSerialService service = acceptServices.get(name
						+ uuidstr);
				if (service != null) {
					service.stop();
				}
			...
			if (action.equals("rfcommWrite")) {
				...
				if (service != null) {
					...
					callbackContext.error("there is no connection on device:"
							+ deviceAddress);
				}
			}
			if (action.equals("rfcommRead")) {
				...
				if (service != null) {
					...
					JSONObject obj = new JSONObject();
					// Tools.addProperty(obj, Tools.DEVICE_ADDRESS,
					// deviceAddress);
					Tools.addProperty(obj, Tools.VALUE,
							Tools.encodeBase64(data));
			...
			if (action.equals("getPairedDevices")) {
				try {
					...
					while (it.hasNext()) {
						...
						Tools.addProperty(obj, Tools.DEVICE_ADDRESS,
								device.getAddress());
						Tools.addProperty(obj, Tools.DEVICE_NAME,
								device.getName());
						ary.put(obj);
					}
		...
		public void onReceive(Context context, Intent intent) {
			if (intent.getIntExtra(BluetoothAdapter.EXTRA_PREVIOUS_STATE, -1) == 11) {
				...
				Tools.addProperty(obj, Tools.DEVICE_ADDRESS,
						device.getAddress());
				Tools.addProperty(obj, Tools.DEVICE_NAME, device.getName());
				Tools.addProperty(obj, Tools.IS_CONNECTED, Tools.IS_TRUE);
				Tools.addProperty(obj, Tools.TYPE, "Classical");
				PluginResult pluginResult = new PluginResult(
						PluginResult.Status.OK, obj);
				pluginResult.setKeepCallback(true);

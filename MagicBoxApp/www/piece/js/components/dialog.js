...
define(['zepto', 'underscore'], function($, _){
    ...
    var Dialog = function(config,btnConfigs){
        ...
        this.btnConfigs = {configs:[{title:'取消'},{title:'确定'}]};
        if(config) {
    ...
    var dialogElem =  '<div id="cube-dialog-wrapper">'
                     ...
                     +   '</div>'
                     +   '<div>'
                     +       '<p class="cube-dialog-subtitle">完成</p>'
    ...
    var btnElem =    '<button class="btn cube-dialog-btn ui-shadow ">'
		                     +    '<span class="ui-btn-inner ui-btn-corner-all">'
		                     +       '<span class="cube-dialog-btn-title ui-btn-text"></span>'
...
//            localBtn.css('width', (btnWidth-15) + 'px');
//            localBtn.css('margin-left', 20 + 'px');
//            localBtn.css('margin-right', 7 + 'px');
//            localBtn.css('margin-bottom', 15 + 'px');
            localBtn.bind('click',function(event){                                   //从this.btnConfigs中调用方法
                var eventName = $(this).attr('eventname');
                // if ($(this).find(".cube-dialog-btn-title").html() == "确定") {
                	// var value = $("#cube-dialog-wrapper").find(".cube-dialog-subtitle").children().val();
                	// me.config.confirm(event, value);
                	// me.hide();
                	// return false;
                // }
           
                if(eventName){
                    btnConfigs[eventName]();
                }
                me.hide();
            })

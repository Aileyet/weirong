define(['zepto', "../base/openapi", "sockjs", "../base/constant","../base/schema"], function($, OpenAPI, SockJs, Cons,Schema) {
	var Util = {
        ...
        sockToObject:function(msg){
                        ...
                        var source=body.substr(8,8);
						var idf=body.substr(16,4);
                        var moduleName=body.substr(20,4);
						var moduleId=body.substr(24,4);
						var actionName=body.substr(28,4);
						var actionId=body.substr(32,4);
						var command=body.substr(36,8);
                        var baoliu=body.substr(44,4);
				  ...
				  var robj=Util.sockToObject(message);
				   if(robj.command=="MesgMesg"){//定时
					 ...
					 if(robj.module=="TMP0"){//温度报警
					  ...
					  else if(robj.idf.substr(2,2)>="60"&&robj.idf.substr(2,2)<="69"){//添加各种模式日程
				         var magicList=Piece.Store.loadObject("magicList", true);
					     var cmds=Piece.Store.loadObject("ModeCmds",true);
					    for(var i=0;i<cmds.length;i++)
					     {
							if(cmds[i].modeIndex==robj.idf
							&&this.SetFormat(cmds[i].gateIndex,2,"l")==robj.idf.substr(0,2)){
								cmds[i].groupIds[cmds[i].groupIds.length]=robj.group;
							}
					    }
...
ConvertToOrder:function(str) {
	...
	//根据魔方编号与设备序列号查找设备
	getCurrentDevice:function(gatewayId,serial){
		var equipList=Piece.Store.loadObject("DeviceList", true);
		for(var i=0;i<equipList.length;i++){
			if(equipList[i].gatewayId==gatewayId&&equipList[i].serial==serial){
				return equipList[i];
			}
		}
	}

define(['zepto','../base/tooltip/tooltip',"../base/templates/template",'../base/util',"../base/openapi"], function($,Tooltip,_TU,Util,OpenApi) {
	var oUl,aLi;
	var disX = 0;
	var disY = 0;
	var minZindex = 1;
	...
	var _Drag={
			//程序入口
			onLoad:function(){
				_Drag.readMenuFromService();
			},
			//加载数据
			onLoadData:function(){
				moveing=false;
				menus = _Drag.getLocal("MeIndexMenus");//获取菜单信息
				if(!(menus!=null && menus.length>0)){
					...
					_Drag.setLocal("MeIndexMenus",menus);
				}
				
				menuPos = _Drag.getLocal("menuPos");//获取位置信息
				if(menuPos == null /*|| _Drag.isEmptyJSON(menuPos)*/){
					$("#ul1").html("");
			...
			initView:function(){
					...
					_Drag.setLocal("menuPos",menuPos);
...
//						$(e.target).parent().find("i").css("display","none");
						...
						_Drag.saveMenuToService(appMenu);
					});
			},
			//绑定事件
...
//				localStorage.removeItem("home_MeIndex_deleteMenuInfo");
				...
				$(obj).find(".close-icon").on("touchend",function(e){
					deleteMenuStr= _Drag.getLocal("home_MeIndex_deleteMenuInfo");
					deleteMenuStr=deleteMenuStr==null?{}:deleteMenuStr;
					...
					_Drag.setLocal("home_MeIndex_deleteMenuInfo",deleteMenuStr);
					_Drag.setLocal("menuPos",menuPos);
					_Drag.setLocal("MeIndexMenus",menus);
					_Drag.onLoadData();
				});
					...
					$(document).on("touchend",function(e){
						...
						if(!isCoincidence){
							...
							if(bool){
								...
								_Drag.setLocal("menuPos",menuPos);//获取位置信息
							}
					...
					Util.AjaxWait(OpenApi.readAllAppMenu,
						...
						function(data, textStatus, jqXHR){
							if(data.appMenuList.length>0 && data.appMenuList[0]!=undefined){
								...
								_Drag.setLocal("MeIndexMenus",appMenu.menus);
								_Drag.setLocal("menuPos",appMenu.menuPos);
								_Drag.setLocal("home_MeIndex_deleteMenuInfo",appMenu.deleteMenu);
								_Drag.onLoadData();
							}else{
								_Drag.onLoadData();
							}
